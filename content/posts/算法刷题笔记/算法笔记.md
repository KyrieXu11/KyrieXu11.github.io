...
class Solution {
    ...
    public List<List<Integer>> threeSum(int[] nums) {
        Arrays.sort(nums);
        dfs(nums, 0, 0, 0, new ArrayList<>());
        return new ArrayList<>(res);
    }

    private void dfs(int[] nums, int k, int t, int curIdx, List<Integer> path) {
        int n = nums.length;
        if (k == 0 && t == 3) {
            res.add(new ArrayList<>(path));
            return;
        ...
        for(int i = curIdx; i < n; i++) {
            ...
            path.add(nums[i]);
            dfs(nums, k + nums[i], t + 1, i + 1, path);
            path.remove(path.size() - 1);
        }
    }
}
...
可以看成两数之和的版本，只不过target变成了`-nums[i]`
...
日期转换问题，需要写出一个数组表示每个月分别多少天

区间求交问题，画个图得出`max(0, min(b, d) - max(a, c))` 而日期问题，需要加1
...
class Solution {
    ...
    public int getDays(String date) {
        ...
        int m = Integer.parseInt(ds[0]);
        int res = 0;
        for(int i = 0; i < m - 1; i++) {
            res += d[i];
        }
        res += Integer.parseInt(ds[1]);
        return res;
    }
}
```
...
class Solution {
    ...
    public boolean dfs(int[] nums, int[] sums, int cur) {
        int n = nums.length;
        int k = sums.length;
        if (cur == n) {
            for (int d: sums) {
...
<font size="6" color="red">这个题解还没想明白（剪枝方法）</font>
...
class Solution {
    public boolean canPartitionKSubsets(int[] _nums, int k) {
        ...
        int[] sums = new int[k];
        Arrays.fill(sums, sum);
        Integer[] nums = Arrays.stream(_nums).boxed().toArray(Integer[]::new);
        // 从小到大排序，方便剪枝
        Arrays.sort(nums, (o1, o2) -> o2 - o1);
        return dfs(nums, sums, 0);
    }

    public boolean dfs(Integer[] nums, int[] sums, int cur) {
        int n = nums.length;
        int k = sums.length;
        if (cur == n) {
            return true;
